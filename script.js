...
function getReadingLevel(score){
    if(score <= 20) return "Low Reader";
    if(score <= 40) return "Medium Reader";
    return "High Reader";
}
...
function showStory() {
  ...
  if (storyIndex < articles.length) {
    ...
    completeSound.play();
    let level = getReadingLevel(score);
    articleBox.innerHTML = `
      <h2>🎉 All Stories Completed!</h2>
      <p>Your Final Score: ${score}</p>
      <h3>📘 Your Reading Level: <span style="color:blue;">${level}</span></h3>
...
function showQuizQuestion() {
  if (quizIndex < allPuzzles.length) {
    ...
    completeSound.play();
    let level = getReadingLevel(quizScore);
    document.getElementById("quiz-section").innerHTML = `
      <h2>🏆 Combined Quiz Completed!</h2>
      <p>Your Final Quiz Score: ${quizScore}</p>
      <h3>📘 Reading Level: <span style="color:blue;">${level}</span></h3>
